...
const ShowMasterAccountStatement = async(req, res) => {
    ...
    const result = await pool.request()
        ...
        .input('inPostUser', req.body.inPostUser)
        .input('inPostIp', req.body.inPostIp)
        .output('outMinPayment', 0)
        .output('outActualBalance', 0)
        .output('outCurrentInterest', 0)
        .output('outPenaultyInterest', 0)
        .output('outQOperationsATM', 0)
        .output('outQOperationsBrand', 0)
        .output('outIdAccountStatement', 0)
        .output('outResultCode', 0)
...
const ShowPhysicalCard = async(req, res) => {
    ...
    const result = await pool.request()
        ...
        .input('inPostIp', req.body.inPostIp)
        .output('outCodeTF', '')
        .output('outCardStatus', '')
        .output('outAccountType', '')
        .output('outExpirationDate', '')
        .output('outResultCode', 0)
